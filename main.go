		...
		case "!stats":
			if msg.Sender.IsModerator && len(tokens) > 1 {
				...
				case "weekly":
					...
					tic.SendChannelMessage(fmt.Sprintf("Weekly Stats: Food was dispensed %d times this week with an average of %f per day, having a max/min/avg force of %f/%f/%f", weeklyStats.Week_total_events, weeklyStats.Daily_avg_events, weeklyStats.Week_max_force, weeklyStats.Week_min_force, weeklyStats.Week_avg_force))
				case "monthly":
					...
					tic.SendChannelMessage(fmt.Sprintf("Monthly Stats: Food was dispensed %d times this Month with an average of %f per day, having a max/min/avg force of %f/%f/%f", monthlyStats.Month_total_events, monthlyStats.Daily_avg_events, monthlyStats.Month_max_force, monthlyStats.Month_min_force, monthlyStats.Month_avg_force))
				}
			}
		}
	}
