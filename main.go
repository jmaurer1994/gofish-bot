		...
		case "!stats":
			if msg.Sender.IsModerator && len(tokens) > 1 {
				...
				case "daily":
					...
					tic.SendChannelMessage(fmt.Sprintf("Daily Stats: Food was dispensed %d times today with a max/min/avg force of %f.0/%f.0/%f.0", dailyStats.Day_event_count, dailyStats.Day_max_force, dailyStats.Day_min_force, dailyStats.Day_avg_force))
				case "weekly":
					...
					tic.SendChannelMessage(fmt.Sprintf("Weekly Stats: Food was dispensed %d times this week with an average of %f.0 per day, having a max/min/avg force of %f.0/%f.0/%f.0", weeklyStats.Week_total_events, weeklyStats.Daily_avg_events, weeklyStats.Week_max_force, weeklyStats.Week_min_force, weeklyStats.Week_avg_force))
				case "monthly":
					...
					tic.SendChannelMessage(fmt.Sprintf("Monthly Stats: Food was dispensed %d times this Month with an average of %f.0 per day, having a max/min/avg force of %f.0/%f.0/%f.0", monthlyStats.Month_total_events, monthlyStats.Daily_avg_events, monthlyStats.Month_max_force, monthlyStats.Month_min_force, monthlyStats.Month_avg_force))
				}
			}
		}
	}
